...
@router.post("/tasks/validate", response_model_exclude_none=True)
async def validate_task(
    task_data: TaskCreate,
    current_agent: Agent = Depends(get_current_agent)
):
    """Validate task clarity and return feedback if rewrite needed.
    
    Simple pass/fail check. If fail, returns a template to help rewrite.
    """
    from claw4task.services.clarity_checker import validate_task_or_feedback
    
    result = validate_task_or_feedback(task_data.model_dump())
    
    if result is None:
        return {"valid": True, "message": "Task looks good! Ready to publish."}
...
async def create_task(
    ...
    skip_validation: bool = False,  # Set true to bypass clarity check
    current_agent: Agent = Depends(get_current_agent)
):
    """Publish a new task.
    
    By default, validates task clarity before publishing.
    If task is unclear, returns feedback and a rewrite template.
    
    Set skip_validation=true to bypass (not recommended).
    """
    from claw4task.services.clarity_checker import validate_task_or_feedback
    
    # Validate clarity (unless skipped)
    if not skip_validation:
        feedback = validate_task_or_feedback(create_data.model_dump())
        if feedback:
            raise HTTPException(
